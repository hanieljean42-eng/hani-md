...
// sharp est optionnel (peut échouer sur Render free tier — binaire natif)
let sharp = null;
try { sharp = require('sharp'); } catch (e) { console.log('[STICKER] sharp non dispo, fallback ffmpeg'); }
const path   = require('path');
const fs     = require('fs');
const os     = require('os');
const { execSync, spawnSync } = require('child_process');

// ── Chemin ffmpeg: système d'abord (apt-get), puis ffmpeg-static optionnel ──
let FFMPEG_BIN = 'ffmpeg'; // système par défaut (Render: apt-get install ffmpeg)
try {
  execSync('ffmpeg -version', { stdio: 'ignore' });
} catch (_) {
  try { FFMPEG_BIN = require('ffmpeg-static') || 'ffmpeg'; } catch (_2) {}
}
...
async function imageToStickerBuffer(inputBuffer, packName, author) {
  ...
  if (sharp) {
    // sharp disponible (local / serveur avec binaires natifs)
    webpBuffer = await sharp(inputBuffer)
      .resize(512, 512, { fit: 'cover', position: 'centre' })
      .webp({ quality: 80, lossless: false })
      .toBuffer();
  } else {
    // Fallback ffmpeg (Render free tier, environnements sans sharp)
    const tmpDir = os.tmpdir();
    const tmpIn  = path.join(tmpDir, `hani_img_${Date.now()}`);
    const tmpOut = path.join(tmpDir, `hani_stk_${Date.now()}.webp`);
    try {
      ...
      const result = spawnSync(FFMPEG_BIN, [
        ...
        '-loop', '1', '-preset', 'picture', '-an', '-y', tmpOut
      ], { timeout: 30000, stdio: ['ignore', 'ignore', 'pipe'] });
      if (result.status !== 0 || !fs.existsSync(tmpOut))
        throw new Error('image→webp échoué: ' + (result.stderr?.toString().slice(-200) || ''));
...
async function stickerToVideo(inputBuffer) {
  ...
  try {
    ...
    const result = spawnSync(FFMPEG_BIN, [
      '-i', tmpIn,
      '-vf', 'scale=512:512:force_original_aspect_ratio=increase,crop=512:512:flags=lanczos',
      '-c:v', 'libx264',
      '-pix_fmt', 'yuv420p',
      '-movflags', '+faststart',
      '-y', tmpOut
...
async function stickerToImage(inputBuffer) {
  ...
  // Fallback ffmpeg
  const tmpDir = os.tmpdir();
  const tmpIn  = path.join(tmpDir, `hani_si_in_${Date.now()}.webp`);
  const tmpOut = path.join(tmpDir, `hani_si_out_${Date.now()}.png`);
  try {
    fs.writeFileSync(tmpIn, inputBuffer);
    const result = spawnSync(FFMPEG_BIN, [
      '-i', tmpIn, '-frames:v', '1', '-y', tmpOut
    ], { timeout: 15000, stdio: ['ignore', 'ignore', 'pipe'] });
    if (result.status !== 0 || !fs.existsSync(tmpOut))
      throw new Error('webp→png échoué');
