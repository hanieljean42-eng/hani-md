...
async function restoreSessionFromId(sessionId, sessionFolder) {
  ...
  try {
    ...
    let jsonString;
    if (isV2) {
      const base64DataV2 = sessionId.replace("HANI-MD-V2~", "");
      const compressed = Buffer.from(base64DataV2, "base64");
      jsonString = zlib.brotliDecompressSync(compressed).toString("utf-8");
    } else {
      const base64DataV1 = sessionId.replace("HANI-MD~", "");
      jsonString = Buffer.from(base64DataV1, "base64").toString("utf-8");
    }
