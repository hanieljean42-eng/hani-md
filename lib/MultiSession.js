...
const { getSelfJid, makeSelfSock, deleteCommandMessage } = require('./selfRedirect');
...
function loadClientEvents(sock, clientId, plan) {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    try {
      ...
      const context = {
        // Toutes les réponses vont dans la discussion "avec soi-même"
        repondre: async (text) => await sock.sendMessage(selfJid, { text }),
      ...
      await cmdInfo.fonction(from, makeSelfSock(sock, from), context);
    } catch (e) {
      console.error(`[MULTI-SESSION] Erreur commande ${command}:`, e.message);
      await sock.sendMessage(selfJid, { text: `❌ Erreur: ${e.message}` });
    }
  });
}
