...
// Données de chaque bot (contacts, chats, groupes)
const botData = new Map(); // name → { contacts: [], chats: [], groups: {} }
...
// qrCallback(qrData) : appelé quand un QR est généré
// pairingPhone : si fourni, demande un code 8 chiffres au lieu du QR
async function startBotInstance(name, sessionId, ownerJid, qrCallback, pairingPhone) {
  ...
  if (sessionId && sessionId.startsWith('HANI-MD~')) {
    ...
    if (!qrCallback) {
      return { success: false, error: `Aucune session pour "${name}". Lance *.addbot ${name}* sans session_id pour obtenir un QR code.` };
    }
    // QR mode — continuer, le QR sera envoyé via qrCallback
  }
  ...
  try {
    ...
    // ── Mode code de couplage ── appelé juste après la création du socket
    if (pairingPhone && !state.creds.registered) {
      setTimeout(async () => {
        try {
          const code = await sock.requestPairingCode(pairingPhone);
          if (qrCallback) await qrCallback(null, code);
        } catch (e) {
          console.log(`[BOT-NET] Erreur pairing code ${name}:`, e.message);
          if (qrCallback) await qrCallback(null, null, e.message);
        }
      }, 3000);
    }

    // ── Stocker contacts/chats pour accès distant ──────────────
    if (!botData.has(name)) botData.set(name, { contacts: [], chats: [] });

    sock.ev.on('contacts.set', ({ contacts }) => {
      const d = botData.get(name) || { contacts: [], chats: [] };
      d.contacts = contacts || [];
      botData.set(name, d);
    });
    ...
    sock.ev.on('contacts.update', (updates) => {
      ...
      for (const upd of (updates || [])) {
        const idx = d.contacts.findIndex(c => c.id === upd.id);
        if (idx >= 0) d.contacts[idx] = { ...d.contacts[idx], ...upd };
        else d.contacts.push(upd);
      }
      botData.set(name, d);
    });
    ...
    sock.ev.on('chats.set', ({ chats }) => {
      const d = botData.get(name) || { contacts: [], chats: [] };
      d.chats = (chats || []).sort((a, b) => (b.conversationTimestamp || 0) - (a.conversationTimestamp || 0));
      botData.set(name, d);
    });
    ...
    sock.ev.on('chats.update', (updates) => {
      const d = botData.get(name) || { contacts: [], chats: [] };
      for (const upd of (updates || [])) {
        const idx = d.chats.findIndex(c => c.id === upd.id);
        if (idx >= 0) d.chats[idx] = { ...d.chats[idx], ...upd };
        else d.chats.unshift(upd);
      }
      botData.set(name, d);
    });
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
      // QR image uniquement si pas de pairingPhone
      if (qr && !pairingPhone && qrCallback) {
        try { await qrCallback(qr); } catch (e) {}
      }
      if (connection === 'open') {
...
// ── Accès aux données d'un bot ──────────────────────────────
function getBotData(name) {
  return botData.get(name) || { contacts: [], chats: [] };
}

// Récupérer les groupes en temps réel via l'API Baileys
async function fetchBotGroups(name) {
  const bot = activeBots.get(name);
  if (!bot || bot.status !== 'connected') return null;
  try {
    return await bot.sock.groupFetchAllParticipating();
  } catch (e) {
    console.log(`[BOT-NET] fetchBotGroups ${name}:`, e.message);
    return null;
  }
}

// Envoyer un message via un bot spécifique
async function sendViaBot(botName, targetJid, content) {
  const bot = activeBots.get(botName);
  if (!bot || bot.status !== 'connected') return { success: false, error: 'Bot non connecté' };
  try {
    await bot.sock.sendMessage(targetJid, content);
    return { success: true };
  } catch (e) {
    return { success: false, error: e.message };
  }
...
// Récupérer les messages récents d'une conversation via un bot
async function fetchBotMessages(botName, jid, count = 20) {
  const bot = activeBots.get(botName);
  if (!bot || bot.status !== 'connected') return null;
  try {
    const msgs = await bot.sock.fetchMessagesFromWA(jid, count);
    return msgs;
  } catch (e) {
    return null;
  }
